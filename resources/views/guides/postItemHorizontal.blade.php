<div class="col-md-12">
  <a href="{{ route('guide_post', ['slug' => $post->slug, 'id' => $post->ID]) }}" title="{{$post->title}}">
    <div class="post mb-4 d-flex flex-md-row flex-column align-items-stretch bg-white">
      ...
      <div class="post-tn post-tn-horizontal lazy" style="background: url('{{ $post->getThumbnail('medium') ?? env('SITE_OG_IMAGE') }}') no-repeat center 20%/cover;" data-bg="url({{ $post->getThumbnail('large') ?? env('SITE_OG_IMAGE') }})">
      </div>
